...
export async function resetAuthDatabase() {
  return withUsersWrite(async () => {
    clearAuthDatabase();
    const { users, credentials } = await buildDefaultAuthUsers();
    const db = { version: 2, updatedAt: null, users };
    await saveUsersDb(db);
    const credPath = await writeDemoCredentials(credentials);
    console.warn(`[auth] SQLite auth database reset — demo credentials: ${credPath}`);
    return db;
  });
}
...
export async function seedDefaultUsersIfEmpty() {
  return withUsersWrite(async () => {
    ...
    await saveUsersDb(db);
    const credPath = await writeDemoCredentials(credentials);
    console.warn(`[auth] Seeded SQLite auth DB (${users.length} users). Credentials: ${credPath}`);
    console.warn(`[auth] Admin: ${credentials.fixedAccounts[0].login} / ${credentials.fixedAccounts[0].password}`);
    console.warn(`[auth] VIP:   ${credentials.fixedAccounts[1].login} / ${credentials.fixedAccounts[1].password}`);
    return db;
  });
}
