...
export async function createUserAdmin(payload) {
  ...
  const user = {
    ...
    avatarUrl: String(payload.avatarUrl ?? `https://api.dicebear.com/7.x/notionists/svg?seed=${username}`).trim(),
    coverUrl: String(payload.coverUrl ?? 'linear-gradient(135deg,#0f172a,#1e293b,#020617)').trim(),
